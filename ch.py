...
# If you will download this, rename it as ch.py once it is already downloaded.
# If not, then save it as ch.py
...
class PM:
  ...
  def addFriend(self, args):
    if args.lower() not in self._friends:
      self.send("wladd", args.lower())
      self._info = list()
      self._friends = list()
      self.send("wl")

  def delFriend(self, args):
    if args.lower() in self._friends:
      self.send("wldelete", args.lower())
      self._info = list()
      self._friends = list()
      self.send("wl")
  ...
  def isOnline(self, args):
    if args.lower() in self._friends:
      for i in self._info:
        if i["user"] == args.lower():
          return i["online"]
    else:
      self.addFriend(args.lower())
      self.sleep(1)
      for i in self._info:
        if i["user"] == args.lower():
          return i["online"]
...
class Room:
  ...
  def __init__(self, room, uid = None, server = None, port = None, mgr = None):
    ...
    self._mgr = mgr
    self._channel = True
    self._modbadge = False
    self._channeldesc = {"blue": 2048,
                         "red": 256,
                         "mod": 32768
                         }
    self._badgedesc = {"1": 64,
                       "2": 128
                       }
  ...
  def setChannel(self, args):
    '''args can be either 'red', 'blue', or 'mod' if modded in group'''
    '''if no channel wanted set args to False or None'''
    if not args:
      self._channel = False
      return True
    elif args.lower() == "red":
      self._channel = "red"
      return True
    elif args.lower() == "blue":
      self._channel = "blue"
      return True
    elif args.lower() == "mod":
      if self.username.lower() in self.mods + [self.owner]:
        self._channel = "mod"
        return True
      else:
        return False
  ...
  def setBadge(self, args):
    '''args = 1 for shield badge, or 2 for staff badge'''
    '''if badge not wanted set args to False or None'''
    if not args:
      self._modbadge = False
      return True
    elif self.username.lower() in self.mods + [self.owner]:
      if str(args) == "1":
        self._modbadge = "1"
        return True
      elif str(args) == "2":
